...
// 实现sort.Iterface，heap.Interface继承了sort.Iterface
...
func main() {
	...
	// 堆操作
	heap.Init(hp)
	fmt.Println("初始堆之后slice: ", hp)

	// push
	heap.Push(hp, Rectangle{100, 10})
	fmt.Println("加入元素之后slice: ", hp)

	// pop
	fmt.Println("top元素：", (*hp)[0])
	fmt.Println("删除并返回最后一个：", heap.Pop(hp)) // 最后一个元素
	fmt.Println("删除元素之后slice: ", hp)
